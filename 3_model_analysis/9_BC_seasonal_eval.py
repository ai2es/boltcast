...
def calc_metrics_per_season_all_rotations(model):
    rotations = [0,1,2,3,4]
    load_dir = '/scratch/bmac87/BC_reviewer_analysis/'
...
def generate_plot2(plot_metric='day_srs'):#for AIES reviewer comments
    seasons = ['summer','fall','winter','spring']
    unet_stats = pickle.load(open('/scratch/bmac87/BC_reviewer_analysis/UNet_season_stats_bar.pkl','rb'))
    lstm_stats = pickle.load(open('/scratch/bmac87/BC_reviewer_analysis/LSTM_season_stats_bar.pkl','rb'))
    ...
    if plot_metric=='day_csi':
        ylims = [.1,.5]
        ytick_nums = [.1,.15,.2,.25,.3,.35,.4,.45,.5]
        ytick_strs = ['0.10','0.15','0.20','0.25','0.30','0.35','0.40','0.45','0.50']
        ylabel = 'Max CSI'
        sublabel_text_x = .02
        sublabel_text_y = .47
        sn_text = .47
    ...
    axes[0].set_title('UNet',fontsize=24)
    axes[1].set_title('LSTM',fontsize=24)
    plt.savefig('./AIES_results_reviewer_edits/seasonal_eval_%s.png'%plot_metric)
    plt.savefig('./AIES_results_reviewer_edits/seasonal_eval_%s.pdf'%plot_metric)
    plt.close()
