...
def parse_args():
    ...
    parser.add_argument('--rotation',type=int,default=1,help='Slurm array index for the job.')
...
def get_outputs(rotation=4):
    ...
    exp_name = 'BC_UNet_rot_%s_lrate_0.000010000_/'%(rotation)
    fname = 'output_labels.pkl'

    unet_output = pickle.load(open(results_dir+exp_name+fname,'rb'))
    y_pred_unet = unet_output['model_output']

    exp_name = 'BC_LSTM_rot_%s_lrate_0.000010000_/'%(rotation)
    lstm_output = pickle.load(open(results_dir+exp_name+fname,'rb'))
...
def generate_variable_model_output(rotation=0,
    ...
    #the test_ds is used to get the labels.
    ...
    for q in range(len(valid_times)):
        if q>=0:
            ...
            save_dir = '/scratch/bmac87/BoltCast_output_movie_pngs/rot_%s/'%(rotation)
            if os.path.isdir(save_dir)==False:
                os.makedirs(save_dir)
            save_file = '%s.png'%(date_str)
            plt.savefig(save_dir+save_file)
            plt.close()

def main():
    rot = parse_args()
    print('generating the model output for rotation:',rot)
    generate_variable_model_output(rotation=rot)
